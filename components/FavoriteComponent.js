...
const mapDispatchToProps = (dispatch) => ({
  deleteFavorite: (petId) => dispatch(deleteFavorite(petId))
});
...
class Favorites extends Component {
  render() {
    if (this.props.pets.isLoading) {
      ...
      return (<Text>{this.props.pets.errMess}</Text>);
    } else {
      const pets = this.props.pets.pets.filter((pet) => this.props.favorites.some((el) => el === pet.id));
      return (
        /*<FlatList data={dishes}
       renderItem={({ item, index }) => this.renderMenuItem(item, index)}
       keyExtractor={(item) => item.id.toString()} />*/
        <Animatable.View animation="fadeInRightBig" duration={2000}>
          <SwipeListView data={pets}
            renderItem={({ item, index }) => this.renderMenuItem(item, index)}
      ...
      <View style={{ alignItems: 'center', backgroundColor: '#DDD', flex: 1, flexDirection: 'row', justifyContent: 'space-between', paddingLeft: 15 }}>
        <TouchableOpacity style={{ alignItems: 'center', justifyContent: 'center', position: 'absolute', top: 0, bottom: 0, right: 0, width: 100, backgroundColor: 'red' }}
          onPress={() => {
            Alert.alert(
              'Delete Favorite?',
              'Are you sure you wish to delete this favorite pet: ' + item.name + '?',
              [
